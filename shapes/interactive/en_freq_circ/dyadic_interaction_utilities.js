/* 
This code is taken mostly unmodified from dyadic_interaction_utilities in the Bowerman & Smith (2022) experiment, found at
https://github.com/kennysmithed/SemanticExtension/tree/main/Experiment1Code/shapes_interaction. The main changes are
cumulative tracking of scores and the display of final score + bonus payment calculation. 
*/
...
// ===== Interaction progress tracker =====
window.inInteractionPhase = false;
window.currentInteractionTrial = 0;
window.totalInteractionTrials = 0;

function updateInteractionProgress() {
  if (!window.inInteractionPhase) return;
  if (window.totalInteractionTrials > 0 && window.currentInteractionTrial > 0) {
    const progress = window.currentInteractionTrial / window.totalInteractionTrials;
...
function interaction_loop() {
  ...
  ws = new WebSocket("wss://emsl.ppls.ed.ac.uk" + my_port_number);
  ...
  //if the received code is valid, use switch to select action based on command_code
    ...
    switch (command_code) {
      ...
      case "EndExperiment": //EndExperiment: you have finished the experiment
        window.inInteractionPhase = false;
        end_experiment(command.final_score); //direct client to the final screen of the experiment
        break;
      ...
      case "Director": //Direct: director trial
        ...
        // command.target_object.object_choices = command.object_choices;
        // director_trial(command.target_object, command.partner_id, command.target_object.picture, command.object_label)
        // // director_trial(command.target_object, command.partner_id);
        // break;
        console.log("Received Director command:", command);
        window.inInteractionPhase = true;
        window.totalInteractionTrials = command.total_trials || 0;
        window.currentInteractionTrial = command.current_trial || 0;
      ...
      case "Matcher": //Matcher: matcher trial
        ...
        // matcher_trial(command.director_label, command.partner_id, command.object_choices)
        // break;
        console.log("Received Matcher command:", command);
        window.inInteractionPhase = true;
        window.totalInteractionTrials = command.total_trials || 0;
        window.currentInteractionTrial = command.current_trial || 0;
        updateInteractionProgress();
      ...
      case "Feedback": //Feedback: feedback for director or matcher
        ...
        display_feedback(command.score, command.c_score, command.label, command.guess, command.target, command.object_label);
        break;
