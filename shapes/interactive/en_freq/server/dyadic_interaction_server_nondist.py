...
# def split_trials(data):
#     circle_trials = []
#     triangle_trials = []

#     for item in data:
#         lexicalization_number = item.get('lexicalization_number')
#         sentence = item.get('sentence')
#         semantics = item.get('semantics')
#         lexicalization = item.get('lexicalisation')

#         circle_picture = item.get('circle picture')
#         if circle_picture:
#             circle_trial = {
#                 'lexicalization_number': lexicalization_number,
...
#                 'shape': 'circle'
#             }
#             circle_trials.append(circle_trial)
#         else:
#             print(f"Warning: Missing 'circle picture' in trial {lexicalization_number}. Skipping this trial.")

#         triangle_picture = item.get('triangle picture')
#         if triangle_picture:
#             triangle_trial = {
#                 'lexicalization_number': lexicalization_number,
#                 'lexicalization': lexicalization,
#                 'sentence': sentence,
#                 'semantics': semantics, 
#                 'picture': triangle_picture,
#                 'shape': 'triangle'
#             }
#             triangle_trials.append(triangle_trial)
#         else:
#             print(f"Warning: Missing 'triangle picture' in trial {lexicalization_number}. Skipping this trial.")

#     split_data = circle_trials + triangle_trials
#     shuffle(split_data)
#     return split_data
...
def collect_prompts(input_list):
    ...
    for item in input_list:
        ...
        picture_path = prepath + item.get('picture') + '.png'
    ...
    for d in prompt_list:
        return_dict.update({k: deepcopy(v) for k, v in d.items()})
    return return_dict
...
def prompt_filter(input_list, shape, sem, desired_n_choices=3):
    input_list = list(set(input_list))
    nuni_items = [item for item in input_list if 'nuni' in item]
    nex_items = [item for item in input_list if 'nex' in item]

    if nuni_items:
        selected_nuni = random.choice(nuni_items)
    elif input_list:
        selected_nuni = random.choice(input_list)
    else:
        return []

    distractors = [item for item in nex_items if item != selected_nuni]
    remaining_choices = random.sample(distractors, min(len(distractors), desired_n_choices - 1))
    final_choices = [selected_nuni] + remaining_choices
    random.shuffle(final_choices)

    while len(final_choices) < desired_n_choices:
        fillers = [item for item in input_list if item not in final_choices]
        if fillers:
            final_choices.append(random.choice(fillers))
        else:
            break
...
##############################
# BIASED TRIAL GENERATOR 
##############################
def generate_trials(trial_bank):

    random.shuffle(trial_bank)
    return trial_bank
            
    # red_square = {'lexicalization_number': 2, 'lexicalization': 'compositional', 
    #                'sentence': 'Not everything in this picture is red', 'semantics': 'not-universal',
    #                'picture': 'square-nuni-red', 'shape': 'square'
    #               }
      
    # blue_square = {'lexicalization_number': 2, 'lexicalization': 'compositional', 
    #                'sentence': 'Not everything in this picture is blue', 'semantics': 'not-universal',
    #                'picture': 'square-nuni-blue', 'shape': 'square'
    #          
            
    # trial_bank = trial_bank[:6]
    # red_trials = []
    # blue_trials = []
    # for t in trial_bank:
    #     if 'red' in t['sentence']:
    #         red_trials.append(t)
    #     else:
    #         blue_trials.append(t)
    
    # split_red = split_trials(red_trials)
    # split_red.append(red_square)
    
    # split_blue = split_trials(blue_trials)
    # split_blue.append(blue_square)
    
#     split_blue = list(filter(
#     lambda item: not (
#         "triangle" in item["shape"].lower() 
#         and "Not everything in this picture is" in item["sentence"]
#     ),
#     split_blue
# ))
    
#     split_red = list(filter(
#     lambda item: not (
#         "triangle" in item["shape"].lower() 
#         and "Not everything in this picture is" in item["sentence"]
#     ),
...
# ))
    ...
    # def multiply_trials(trials):
        
    #     output_trials = []
    #     for trial in trials:
    #         n = 6
    #         if trial['sentence'].startswith("Nothing"):
    #             if trial['shape'] == 'triangle':
    #                 duplicates = [trial.copy() for _ in range(n)]
    #             else:
    #                 #circles
    #                 duplicates = [trial.copy() for _ in range(int(n/3))]
    #             for d in duplicates:
    #                 output_trials.append(d)
                    
    #         elif trial['sentence'].startswith('Everything'):
    #             if trial['shape'] == 'triangle':
    #                 #GOOD
    #                 if trial['semantics'] == 'not-exist':
    #                     duplicates = [trial.copy() for _ in range(n)]
    #                 else:
    #                     #nuni everything-not 
    #                     duplicates = [trial.copy() for _ in range(int(n/2))]
    #             else:
    #                 n = int(n/3)
    #                 if trial['semantics'] == 'not-exist':
    #                     duplicates = [trial.copy() for _ in range(n)]
    ...
    # mult_blue = multiply_trials(split_blue)
    # mult_red = multiply_trials(split_red)
    
    # shuffled_targets = list(chain.from_iterable(zip(mult_blue, mult_red)))
...
def ping_all(list_of_ids):
    for client_id in list_of_ids:
        if client_id in global_participant_data and 'client_info' in global_participant_data[client_id]:
            send_message_by_id(client_id, {"command_type": "Ping"})
        else:
            print(f"[WARNING] Skipping ping to {client_id}: ** client_info not set. ** ")
...
def enter_phase(client_id, phase):
    ...
    if phase == 'Start':
        ...
        trial_path = os.path.join('..', 'frequency-trials-nosquares-mod-nondist.json')
...
def handle_client_response(client_id,response_code,full_response):
    ...
    # if response_code == 'CLIENT_INFO': (changed this to elif on 31/10/2025 based on Gemini suggestion)
    if response_code == 'CLIENT_INFO':
        global_participant_data[client_id]['participantID'] = full_response['client_info']
        if 'partner' in global_participant_data[client_id]:
            partner_id = global_participant_data[client_id]['partner']
            both_ready = (
                'participantID' in global_participant_data.get(partner_id, {}) and
                'participantID' in global_participant_data[client_id]
            )
            if both_ready:
                progress_phase(client_id)
                progress_phase(partner_id)
...
def start_interaction_trial(list_of_participants):
    ...
    if not(all_connected(list_of_participants)):
        ...
        if trial_counter >= ntrials:
            ...
            # get the shared image choices (filtered_targets)
            trial_matcher_targets = global_participant_data[director_id]['matcher_targets'][trial_lex_number]
            shape = target['shape']
            semantics = target['semantics']
            filtered_targets = prompt_filter(trial_matcher_targets, shape, semantics)

            global_participant_data[director_id]['last_object_choices'] = filtered_targets

            for c in list_of_participants:
                this_role = global_participant_data[c]['role']
                if this_role == 'Director':
                    instruction_string = {
                        "command_type": "Director",
                        "target_object": target,
                        "partner_id": matcher_participant_id,
                        "lexicalization_number": trial_lex_number,
                        "object_choices": filtered_targets, #,  # send image set to director,
                        "current_trial": trial_counter + 1, # added to create a counter in the interaction phase
                        "total_trials": ntrials # added to create a counter in the interaction phase
                    }
                    print("Sending to director_id {}: {}".format(director_id, instruction_string))
                else:
                    instruction_string = {
                        "command_type": "WaitForPartner",
                        "total_trials": ntrials,
                        "current_trial": trial_counter + 1
                    }
                send_message_by_id(c, instruction_string)
...
def handle_director_response(director_id, director_response):
    ...
    trial = global_participant_data[director_id]['trial_list'][trial_n]
    semantics = trial['semantics']
    director_shape = trial['shape']
    ...
    trial_matcher_targets = global_participant_data[director_id]['matcher_targets'][trial['lexicalization_number']]
    filtered_targets = prompt_filter(trial_matcher_targets, director_shape, semantics)
    ...
    send_message_by_id(
        matcher_id,
        {
            ...
            "object_choices": filtered_targets,
...
def handle_matcher_response(matcher_id,matcher_response):
    ...
    if not(all_connected([director_id,matcher_id])):
        ...
        print(f" ////////// {matcher_response} /////////")
        guess = matcher_response['response']
        print(f"LABEL: {label} ***************** GUESS: {guess}")

        def check_guess(f1, f2):
            f1_fname = f1.strip()
            f2_fname = re.search(r'src="(?:\.\./)+assets/([^"]+)"', f2).group(1)
            return f1_fname in f2_fname
        ...
        feedback = {"command_type":"Feedback","score":score,
                    "target":target,"label":label,"guess":guess, 'c_score': global_participant_data[matcher_id]['score'], 'object_label': matcher_response['director_label']}
...
PORT=9005 #this will run on port 900X
