...
var jsPsych = initJsPsych({
  ...
  message_progress_bar: 'Progress in this phase',
  on_finish: function () {
    jsPsych.data.displayData("csv"); 
  },
});

// This code will prompt a user with the browser's native "confirm leave"
// dialog if they try to refresh or close the page.
...
async function fetchTrialData() {
  ...
  // const zopResponse = await fetch('frequency-trials-nosquares.json');
  const zopResponse = await fetch('frequency-trials-nosquares-mod-nondist.json');
...
//Function to generate frequency biased trials
function generateBiasedObservationTrials(trialBank) {
  return jsPsych.randomization.shuffle(trialBank);
}
...
window.generateBiasedObservationTrials = generateBiasedObservationTrials

biasedTrials = generateBiasedObservationTrials(trial_bank)
...
var observation_trials = build_observation_phase(biasedTrials);
...
function waiting_for_partner() {
  ...
  var waiting_trial = {
    type: jsPsychHtmlButtonResponse,
    stimulus: "Waiting for partner...Please don't refresh/reload the tab!",
    choices: [],
    on_finish: function () {
      jsPsych.pauseExperiment();
    },
...
function end_waiting() {
  if (
    jsPsych.getCurrentTrial().stimulus == "Waiting for partner...Please don't refresh/reload the tab!" ||
...
// copied from https://emsl.ppls.ed.ac.uk/~wuegaki/dp_interaction/shapes/_weighted_interactive/zop_followup/dyadic_interaction.js
...
function director_trial(target_object, partner_id) {
  ...
  const subtrial2 = {
    ...
    stimulus: function () {
      ...
      // remove progressText from here to avoid duplicate
      const imageHTML = imageChoices.map(imgSrc => {
        ...
        return `<img src="${fullPath}" 
                     style="width: 400px; height: auto; margin: 10px; padding: 5px; 
                            ${isTarget ? 'opacity: 1;' : 'opacity: 0.2;'}">`;
      }).join("");

      const prompt1 = "<p><small>Select a sentence matching the highlighted picture to send to your partner. Your partner will choose a picture from the set of options you see here.</small></p>";
    ...
    on_load: function() {
      // ✅ Single source of truth for displaying counter
      if (window.inInteractionPhase && window.currentInteractionTrial && window.totalInteractionTrials) {
        const counterDiv = document.createElement("div");
        counterDiv.textContent = `Trial ${window.currentInteractionTrial} of ${window.totalInteractionTrials}`;
        counterDiv.style.cssText = "text-align:right; font-size:16px; margin-bottom:5px;";
        const stim = document.querySelector(".jspsych-content") || document.body;
        stim.prepend(counterDiv);
      }
    },
    on_finish: function (data) {
...
//       const progressText = (window.totalInteractionTrials && window.currentInteractionTrial)
//       ? `<div style="text-align:right; font-size:16px; margin-bottom:5px;">
//          Trial ${window.currentInteractionTrial} of ${window.totalInteractionTrials}
//        </div>`
//       : "";
...
function matcher_trial(label, partner_id, object_choices) {
  ...
  const prompt1 = "<p><em>Select the picture your partner is describing:</em></p>";
  const imageChoices = object_choices.map(imageSrc => {
    return `<img src="${imageSrc}" style="width:400px; height:auto;">`;
  });
  ...
  const trial = {
    type: jsPsychHtmlButtonResponse,
    stimulus: function() {
      // Keep stimulus simple and valid HTML
      return `
        <div style="text-align:center;">
          ${prompt1}
          <div style="margin-top:10px; font-size:20px;">${label}</div>
        </div>
      `;
    },
    ...
    on_start: function(trial) {
      const shuffled_object_choices = jsPsych.randomization.shuffle(trial.choices);
      trial.choices = shuffled_object_choices;
      trial.data = { button_choices: shuffled_object_choices };
    },
    on_load: function() {
      // ✅ Show progress counter here too
      if (window.inInteractionPhase && window.currentInteractionTrial && window.totalInteractionTrials) {
        const counterDiv = document.createElement("div");
        counterDiv.textContent = `Trial ${window.currentInteractionTrial} of ${window.totalInteractionTrials}`;
        counterDiv.style.cssText = "text-align:right; font-size:16px; margin-bottom:5px;";
...
//        </div>`
//       : "";
//   return progressText + prompt1.concat('\n', label);
// },
//     choices: imageChoices,
//     button_html:
...
function display_feedback(score, c_score, label, guess, target, object_label) {
  ...
  function revealQuadrants() {
    ...
    setTimeout(() => { document.getElementById("quadrant-2").style.opacity = 1; }, 2500); // BL
    setTimeout(() => { document.getElementById("quadrant-3").style.opacity = 1; }, 5000); // TR
    setTimeout(() => { document.getElementById("quadrant-4").style.opacity = 1; }, 7500); // BR
  }
...
var consent_screen = {
  ...
  stimulus: "<h3>Welcome to the experiment</h3>" +
    ...
    "This is an experiment about language learning. It will take about 30-40 minutes to complete. " +
...
var instruction_screen_observation = {
  ...
  Now it's time to learn. A sequence of pictures and their labels will appear on your screen. You cannot interact with the experiment at this time, so relax and watch closely.",
  choices: ["Continue"],
};
...
var instruction_screen_enter_waiting_room = {
  ...
  so please progress through the experiment in a timely fashion, and please if at all possible <span style='color:red'><b>do not abandon or refresh/reload the experiment, \
  or click away from the tab</b></span>, since this will also end the experiment for your partner.</p>",
  choices: ["Continue"]
};
  ...
  var instruction_screen_interaction = {
    ...
    pages: [
      ...
      <p style='text-align:left'> \
          ...
          This phase consists of <span style='color:blue'><b>80</b></span> iterations, meaning you will be the RECEIVER and the SENDER <b>40 times each</b>.\
      </p>",
      "<h3>Pre-interaction Instructions: Scoring</h3> \
      <p style='text-align:left'> \
          ...
          You will probably get some wrong at first, so watch the feedback and try to learn from it!<br><br> \
           Again, please if at all possible <span style='color:red'><b>do not abandon or refresh/reload the experiment, \
  or click away from the tab</b></span>, since this will also end the experiment for your partner.</p>"
    ],
